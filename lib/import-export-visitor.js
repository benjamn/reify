...
class ImportExportVisitor extends Visitor {
  ...
  visitExportAllDeclaration(path) {
    ...
    const hoistedCode = pad(
      ...
      this,
      // Note that module.copyNamespace is already bound to the module
      // object, so we can use it as a setter function here.
    ...
    // module, and uses the module.watch API, so it should be hoisted as
    // if it was an import declaration.
    hoistImports(this, path, hoistedCode);
  }
  ...
  visitExportNamedDeclaration(path) {
    ...
    if (decl.specifiers) {
      ...
      if (decl.source) {
        ...
        // source module, and uses the module.watch API, so it should be
