...
module.exports = class ImportExportVisitor extends Visitor {
  ...
  addExportedLocalNames(specifierMap) {
    if (specifierMap) {
  ...
  hoistImports(importDeclPath, hoistedCode, childName) {
  ...
  pad(newCode, oldStart, oldEnd) {
    if (this.code) {
      ...
      const lineEnd = endCode === carriageReturnCode ? "\r\n" : "\n";
