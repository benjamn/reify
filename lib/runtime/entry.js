...
Entry.get = function (exported) {
  if (utils.isObjectLike(exported) &&
      weakEntryMap.has(exported)) {
    return weakEntryMap.get(exported);
  }
  return null;
};
...
function syncExportsToNamespace(entry) {
  ...
  if (utils.isObject(entry.exports)) {
    var exportsKeys = safeKeys(entry.exports);
    var exportsKeyCount = exportsKeys.length;
    for (var i = 0; i < exportsKeyCount; ++i) {
      var key = exportsKeys[i];
      if (typeof Object.getOwnPropertyDescriptor === "function") {
        var desc = Object.getOwnPropertyDescriptor(entry.exports, key);
        if (hasOwn.call(desc, "value")) {
          // If entry.exports[key] is a simple property, simply copy its
          // value over to entry.namespace[key].
          entry.namespace[key] = desc.value;
        } else {
          // Avoid triggering getters until necessary.
          desc.configurable = true;
          Object.defineProperty(entry.namespace, key, desc);
        }
      } else {
        entry.namespace[key] = entry.exports[key];
      }
    }
  }
}
